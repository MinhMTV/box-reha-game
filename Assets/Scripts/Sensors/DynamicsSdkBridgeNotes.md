...
- Uncalibrated sensor `Power=1` is neutral gameplay damage; `NormalizationValid=false` until a matching saved Strong reference is applied by CalibrationLibrary (implemented; capped at 2.5, still UNQUALIFIED). `RawForce=0` is a legacy compatibility field, not a measurement.
...
1. Explicit COMPATIBILITY mode builds the native collector and Unity Android player using resource 2.12.0 plus the pinned Java/reflection adapter. VENDOR-UNCHANGED still lacks resource 2.11.1; official corrected metadata/artifacts remain a vendor request. See ../../../SDK_COMPATIBILITY_REPORT.md.
2. Match every build/test artifact to its recorded source and APK hash. Current candidate identity is in ../../../ANDROID_STATUS.md; compile success does not establish physical JNI/BLE behavior.
...
`Assets/Editor/DynamicsAndroidGradleIntegration.cs` configures only the generated export: it supplies Kotlin 2.3.21, adds the authoritative Maven repository (environment override `DYNAMICS_MAVEN_PATH`), ensures API 36 or newer in both Unity modules, and applies the documented AGP 9 external-Kotlin opt-outs. It preserves Unity's AGP/Gradle and rejects unsupported or unresolved configuration. AGP 7 from the old Unity 2022 default toolchain is deliberately rejected; the installed Unity 6000.6.0f1 toolchain has existing successful compatibility build evidence. [Android AGP 9 migration documentation](https://developer.android.com/build/migrate-to-built-in-kotlin) explains the temporary `android.builtInKotlin=false` / `android.newDsl=false` settings. The host tests under `tests/android-export` check synthetic generated-file transformations only.

## Sensor lifecycle update — 2026-09-18

Both apps use the same verified public SDK 0.25.6 JVM adapter for delete/swap/raw value-class signatures. Pair returns the actual PeripheralId and confirms both saved and runtime state. Resource internals are not reflected. Unity auto-selects a valid discovery, preserves explicit multi-device selection, and displays saved offline sensors even when absent from observeGloves. Scanner status comes from scannerState. Remove stops scanning and waits for saved/runtime absence. Side changes wait for both views. ALPHA and DELTA use identical management paths; newly identified mixed-family additions are removed. The production same-family one-pair policy remains.

Default non-recording background timeout is one second. Inspector supplies explicit RAW versus COMPUTED modes, per-ID packet-list observation, complete schema-2 raw arrays, diagnostics, deduplicated computed events, partial-file buffered recording and manual annotations. See tools/DynamicsSdkInspector/README.md at the project root for recording/ABI details. DELTA retains powerIndex semantics and the existing Kick mapping; no hardware validation is implied by compilation.
