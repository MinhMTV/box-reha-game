...
/// <summary>
/// Sole owner of the F1 debug overlay: diagnostics/development-only information (FPS, game/sensor
/// input state, spawner pacing internals, active target count). Never gameplay-relevant; disabled
/// outside development builds. Attach to Canvas in Game scene.
/// </summary>
public class DebugUI : MonoBehaviour
{
    ...
    private InputProviderRouter router;

    void Start()
    {
        isVisible = visibleByDefault && Debug.isDebugBuild;
        spawner = FindAnyObjectByType<TargetSpawner>();
        router = FindAnyObjectByType<InputProviderRouter>();
        if (debugText != null)
    ...
    private void UpdateDisplay()
    {
        ...
        string providerStatus = ResearchSessionLog.Error ?? (SessionInputSelection.Physical
            ? AndroidDynamicsController.EnsureInstance().Notice
            : router != null ? router.GetStatusLine() : "No input provider");
