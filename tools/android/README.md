...
Pinned qualification toolchain: Java 17, Gradle 8.13, Android Gradle Plugin 8.13.2, Kotlin 2.3.21. Dependencies use the vendor-supplied `mavenLocal` directory plus Google and Maven Central. VENDOR-UNCHANGED performs no substitution; explicit COMPATIBILITY substitutes resource 2.12.0 and uses the pinned adapter. Build output stays under `tools/android/build`; the existing tracked plugin `.gradle` files are not used.

From the game root:

```powershell
.\scripts\verify-android-collector.ps1 -GradlePath 'C:\path\to\gradle-8.13\bin\gradle.bat' -SdkMode COMPATIBILITY
```
...
VENDOR-UNCHANGED remains blocked by the supplied SDK dependency/metadata issues. COMPATIBILITY has an existing successful native build and 16 JVM tests, plus a separately recorded Unity APK. These are dated results, not fresh execution in this documentation update. [Current Android identity](../../ANDROID_STATUS.md), [compatibility details](../../SDK_COMPATIBILITY_REPORT.md), [remaining hardware work](../../TODO.md).
